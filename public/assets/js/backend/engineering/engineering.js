define(['vue','jquery', 'bootstrap', 'backend', 'table', 'form', 'upload','echarts', 'template', 'echarts-theme', 'summernote'], function (Vue,$, undefined, Backend, Table, Form, Upload, Echarts, Template) {
    var Controller = {
        operate: {
            ...
            'click .btn-delitem': function (e, value, row, index) {
                ...
                Layer.confirm(
                    ...
                    function (index) {
                        $.ajax({
                            type: "POST",
                            url: 'engineering/project/del/ids/' + row.id,
                            dataType: 'json',
                            error: function (re) {
                                Toastr.error('操作失败');
                            },
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        // {checkbox: true},
                        { field: 'name', title: '工程名称', formatter: Controller.api.formatter.project, operate: 'like' },
                        { field: 'city', title: '区域', formatter: Controller.api.formatter.address, operate: false },
                        { field: 'plan_file_file', title: '方案文件', formatter: Controller.api.formatter.url, operate: false },
                        { field: 'createtime', title: '创建时间', addclass: 'datetimerange', operate: false, formatter: Table.api.formatter.datetime },
                        {
                            field: 'operate', width: '170px', title: '操作', table: table, events: this.operate, buttons: [
                                ...
                                //         url: 'engineering/engineering/planFile'
                                //     }
        ...
        detail: function () {
            ...
            // 获取图片高宽
            var imgSrc = Config.item_map_image;
            var img = new Image();
            img.src = imgSrc;
            var img_k = 0;
            // 完全加载完毕的事件
        ...
        api: {
                ...
                table.bootstrapTable({
                    ...
                    columns: [
                        [
                            ...
                            // {field: 'createtime', title: __('创建时间'), operate: false, visible: false,
                            //     formatter : function(value, row, index, field){
                            //         return "<span style='display: block;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;' title='" + row.address + "'>" + value + "</span>";
                            //     },
                            //     cellStyle : function(value, row, index, field){
                    ...
                    onLoadSuccess: function () {
                        ...
                        $('#table1 tbody tr').eq(1).mouseleave(function () {
                            let index = $('#table1 tbody tr td')
            ...
            chart2: function (engineering_id) {
                ...
                $.ajax({
                    type: "POST",
                    url: "engineering/engineering/type_census",
                    data: { 'engineering_id': engineering_id },
                    dataType: 'json',
                    error: function (re) {
                        myChart.setOption(option);
                    ...
                    columns: [
                        {
                            ...
                            formatter: function (value, row, index) {
                                if ($("#table1").data('operateData')) {
                                    ...
                                    return '<a href="' + url + '" title="' + value + '" class="btn-addtabs">' + row.mon_type_name + '</a>';
                                } else {
            ...
            searchList: {//渲染的方法
                status: function (value, row, index) {
                    return '<select class="form-control" name="status"><option value="" selected>全部</option><option value="0" >进行中</option><option value="1">已完工</option></select>';
                },
            },
            formatter: {
                status: function (value, row, index) {
                ...
                url: function (value, row, index) {
                    return '<div class="input-group input-group-sm" style="width:60px;"><input type="text" class="form-control input-sm" value="' + value + '"><span class="input-group-btn input-group-sm"><a href="' + value + '" target="_blank" class="btn btn-default btn-sm"><i class="fa fa-link"></i></a></span></div>';
                },
            }
        }
    };
