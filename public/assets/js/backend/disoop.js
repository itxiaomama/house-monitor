        ...
        index: function () {
            ...
            new Vue({
                ...
                mounted() {
                    ...
                    $.ajax({
                        type: 'get',
                        url: '/public/JMRIUsnmLz.php/disoop/getAreaInfo',
                        dataType: 'json',
                        success: function (res) {
                ...
                methods: {
                    ...
                    IntMap() {
                        ...
                        $.getJSON('/public/assets/js/json/xiaoshan.json', function (geoJSON) {
                            that.townshipList = []
                            var geojson = new AMap.GeoJSON({
                                ...
                                getPolygon: function (geojson, lnglats) {
                                    ...
                                    that.calculateCenter(latarr)
                                    var lat = []

                                    lat.push(that.mapCenter.R,that.mapCenter.Q)
                                    // console.log(that.mapCenter)
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '/public/JMRIUsnmLz.php/disoop/Inspectpercentage',
                            dataType: 'json',
                            data: { 'area': '萧山区' },
                            success: function (res) {
                    ...
                    calculateCenter(lnglatarr) {
                       ...
                       this.mapCenter=(new AMap.LngLat((Lng * 180) / Math.PI, (Lat * 180) / Math.PI))
                        ...
                        if (bounds) {
                            ...
                            for(let i=0;i<bounds.length;i++){
                                for(let j=0;j<bounds[i].length;j++){
                                    var lal=[bounds[i][j].R,bounds[i][j].Q]
                                    // console.log(lal)
                                    arr.push(lal)
                             }
                           
                            }
                            that.calculateCenter(arr)
                            var lat = []

                            lat.push(that.mapCenter.R,that.mapCenter.Q)

                            var markerContent = "<div class='mapcityName'>" + data.name + "</div>"
                            ...
                            for (let i = 0, l = bounds.length; i < l; i++) {
                                ...
                                that.polygons.push(polygon)//除萧山外其他地区的绘制多边形
                        ...
                        AMap.plugin('AMap.Geocoder', function () {
                            ...
                            geocoder.getLocation(addressArr, function (status, result) {
                                ...
                                if (status === 'complete' && result.info === 'OK') {
                                    ...
                                    var markerContent = "<div class='mapcityName'>" + address + "</div>"
                                    that.marker = new AMap.Marker({
                                        position: lal,
                                        // 将 html 传给 content
                                        content: markerContent,
                    ...
                    handlePolygon(result) {
                        ...
                        for (let i = 0; i < that.geoJSON.length; i++) {
                            if (result === that.geoJSON[i].properties.name) {
                                console.log(that.geoJSON[i].geometry.coordinates)
                                for(let j=0;j<that.geoJSON[i].geometry.coordinates[0].length;j++){
                                    var lal=[that.geoJSON[i].geometry.coordinates[0][j].R,that.geoJSON[i].geometry.coordinates[0][j].Q]
                                    arr.push(lal)
                                ...
                                AMap.convertFrom(that.geoJSON[i].geometry.coordinates[0], 'gps', function (status, result) {
                                    ...
                                    var markerContent = "<div class='mapcityName'>" + that.cityName + "</div>"
                                    that.marker = new AMap.Marker({
                                        position: lat,
                                        // 将 html 传给 content
                                        content: markerContent,
                                    ...
                                    // 将 markers 添加到地图
                                    that.map.add(that.marker);
                                    // var markerContent = "<div>" + that.cityName + "</div>"
