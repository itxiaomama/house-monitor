...
//        var_dump($filter, $op);die;
    ...
    {
        if (!$this->dataLimit) {  //未开启数据限制功能
            return null;
        }
        if ($this->auth->isSuperAdmin()) {  //超级管理员 默认不开启数据限制功能
            return null;
        }
        $adminIds = [];
    ...
    /*
     * 将权限验证封装 copy代码
     * time：2022年4月2日10:05:26
     * author：wqh
     */

    public function authMethod($authStaff= false){
    ...
    public function parameterValidation($param,$request){

        $request = array_keys(array_filter($request));
