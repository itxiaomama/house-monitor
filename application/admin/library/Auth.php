...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        //当前账号所属的机构是否已过期
        $ruleIds = $this->getRuleIds($admin->id);
        if(!in_array('*', $ruleIds) ){
            //找到员工
            /*
            ...
            $staff = Db::name('staff')
                ->field('id,agency_id')
                ->where('id',  $engineering_staff['staff_id'])
                ->find();

            //找到机构
            $agency = Db::name('agency')
                ->field('id,agency_name,cs_end_time')
                ->where('id',  $staff['agency_id'])
                ->find();
            */

            /*2022年3月30日09:59:47 WQH 修改查询账号有效期 START*/
            $agency = Db::name('staff')
                ->alias('sf')
                ->join('jc_agency jy','sf.agency_id = jy.id')
                ->field('sf.id,cs_end_time')
                ->where(['sf.admin_id'=>$admin->id])
                ->find();
            /*2022年3月30日09:59:47 WQH 修改查询账号有效期 END*/

            //是否超过有效期的结束事件
            if(is_null($agency['cs_end_time']) || time() > strtotime($agency['cs_end_time']) ){
                $this->setError('该账号的有效期已到，请联系平台方');
                return false;
            }
