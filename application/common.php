...
if (!function_exists('array_select')) {
     ...
     * @param string $value 需求的值
     */
    function array_select($resultSet,$value)
    {
        $arr = [];
...
if(!function_exists('lcg_rand_float')){
    /**
     * 获取随机数
     * @param array $resultSet 最小值
     * @param string $value 最大值
     */
    function lcg_rand_float ($min=0,$max=1) {
...
if (!function_exists('responseJson')) {
     ...
     */
    function responseJson(int $err_code = 1, string $message = "success", $data = array())
    {
        $response = [
            'status'  => $err_code,
            'message' => $message,
        ];
        if (!empty($data) || $err_code = 0) {
